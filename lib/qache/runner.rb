...
module Qache
  class Runner
    ...
    def initialize
      ...
      pid = @process.running?
      if pid
        STDERR.puts "There is already a Qache process running (pid #{pid}), exiting."
        exit(1)
      elsif pid.nil?
    ...
    def load_config_file(filename)
      YAML.load(File.open(filename))["qache"].each do |key, value|
        # alias some keys
        case key
    ...
    def parse_options
      self.options = { :host => "127.0.0.1",
                       :port => 22122,
                       :path => File.join("", "var", "spool", "qache"),
                       :log_level => Logger::INFO,
                       :daemonize => false,
                       :timeout => 0,
                       :pid_file => File.join("", "var", "run", "qache.pid") }

      OptionParser.new do |opts|
        opts.summary_width = 25

        opts.banner = "Qache (#{QacheServer::VERSION})\n\n",
                      "usage: qache [options...]\n",
                      "       qache --help\n",
                      "       qache --version\n"
        ...
        opts.on("-q", "--queue_path PATH",
                :REQUIRED,
                "Path to store Qache queue logs", "(default: #{options[:path]})") do |queue_path|
        ...
        opts.on(
          ...
          "--port PORT", 
          Integer, 
          "TCP port on which to listen (default: #{options[:port]})") do |port|
          options[:port] = port
        end
        ...
        opts.on(
          "-PFILE", 
          "--pid FILENAME", "save PID in FILENAME when using -d option.", 
          "(default: #{options[:pid_file]})") do |pid_file|
        ...
        opts.on_tail("-V", "--version", "Print version number and exit.") do
          puts "Qache #{QacheServer::VERSION}\n\n"
          exit
        end
      end.parse!
    end
    ...
    def start
      ...
      @server = QacheServer::Base.new(options)
      @server.run
    ...
    def shutdown
      begin
        ...
        QacheServer::Base.logger.info "Shutting down."
        @server.stop
      rescue Object => e
